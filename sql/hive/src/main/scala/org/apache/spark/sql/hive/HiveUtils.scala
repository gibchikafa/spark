...
private[spark] object HiveUtils extends Logging {
  ...
  val HIVE_METASTORE_VERSION = buildStaticConf("spark.sql.hive.metastore.version")
    .doc("Version of the Hive metastore. Available options are " +
      ...
      "Note: this Hops Spark build is compiled directly against Hops Hive " +
      "4.1, so with the bundled (builtin) jars only 4.1.x actually works; " +
      "older versions would require compatible external metastore jars.")
    .version("1.4.0")
    .stringConf
